...
#define MCE_VERSION "1.13.2-Alpha"
...
char OldMapFilePath[PLATFORM_MAX_PATH];
...
public void OnMapStart()
{
	...
	g_Config.Rewind();
	LoadOldMapListFromFile();
}

public void LoadOldMapListFromFile()
{
	if(h_OldMapList)
		delete h_OldMapList;

	BuildPath(Path_SM, OldMapFilePath, sizeof(OldMapFilePath), "configs/old_map_file.cfg");

	h_OldMapList = CreateKeyValues("Old_Map_List_Mapchooser");

	if(!FileExists(OldMapFilePath)) KeyValuesToFile(h_OldMapList, OldMapFilePath);
	else FileToKeyValues(h_OldMapList, OldMapFilePath);

	LogMessage("Found Old Map File: \"%s\"", OldMapFilePath);
}
...
public void OnMapEnd()
{
	...
	if(h_OldMapList == INVALID_HANDLE)
	{
		LoadOldMapListFromFile();
	}

	if(!KvJumpToKey(h_OldMapList, map))
	{
		KvRewind(h_OldMapList);
		KvJumpToKey(h_OldMapList, map, true);
		KvSetNum(h_OldMapList, "Cooldown", Cooldown+1);
		KvRewind(h_OldMapList);
		KeyValuesToFile(h_OldMapList, OldMapFilePath);
	}
	KvRewind(h_OldMapList);
	
	if(KvGotoFirstSubKey(h_OldMapList))
	{
		do
		{
			Cooldown = KvGetNum(h_OldMapList, "Cooldown", 0);
			Cooldown--;
			KvSetNum(h_OldMapList, "Cooldown", Cooldown);
			if (Cooldown <= 0)
			{
				deleteEmpty = true;
			}
		} while (KvGotoNextKey(h_OldMapList));
	}
	KvRewind(h_OldMapList);
	...
	if (deleteEmpty)
	{
		for ( ; ; )
		{
			if (!KvGotoFirstSubKey(h_OldMapList))
			{
				...
				if (Cooldown <= 0)
				{
					if (KvDeleteThis(h_OldMapList) == -1)
					{
						break;
					}
					else
...
void CreateNextVote()
{
	...
	if(GetConVarInt(g_Cvar_ExcludeMaps) && GetArraySize(tempMaps) > GetConVarInt(g_Cvar_ExcludeMaps))
	{
		if(KvGotoFirstSubKey(h_OldMapList))
		{
			do
			{
				KvGetSectionName(h_OldMapList, map, sizeof(map));
				RemoveStringFromArray(tempMaps, map);
			} while (KvGotoNextKey(h_OldMapList));
			KvRewind(h_OldMapList);
		}
...
public int Native_ExcludeMap(Handle plugin, int numParams)
{
	...
	else if(KvJumpToKey(h_OldMapList, map))
	{
		KvSetNum(h_OldMapList, "Cooldown", Cooldown);
		KvRewind(h_OldMapList);
		KeyValuesToFile(h_OldMapList, OldMapFilePath);
